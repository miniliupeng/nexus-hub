...
本项目旨在打破传统小作坊式“Gin + GORM 简单增删改查”的初学者范式，完全按照一线大厂（字节跳动、美团、腾讯、阿里）核心业务中台的技术标准与规范实施落地。
...
3. **前后端深度闭环亲和性**：由前端视角驱动后端 API 契约设计，完美处理跨域（CORS）、双 Token 无感静默刷新、统一状态码与 TypeScript SDK / OpenAPI 3.0 契约；
